...
class MainWindowNav(Gtk.ApplicationWindow):
    def __init__(self, loader, saver):
        ...
        self.btn_add_property = Gtk.Button(label="Add Property")
        self.btn_add_property.connect("clicked", lambda w: self._add(Property, "prop"))
        add_box.pack_start(self.btn_add_property, False, False, 0)
    ...
    def _update_ui(self):
        ...
        self.btn_add_sub.set_visible(False)
        self.btn_add_arg.set_visible(False)
        self.btn_add_script.set_visible(False)
        self.btn_add_property.set_visible(False)

        if isinstance(self.current_command, Command):
            self.btn_add_sub.set_visible(True)
            self.btn_add_arg.set_visible(True)
            self.btn_add_script.set_visible(True)
        ...
        if isinstance(self.current_command, Argument):
            self.btn_add_property.set_visible(True)
            self.btn_add_property.set_sensitive(active)
